...
fn puzzle_a(input: &str) -> String {
    ...
    let points_by_wire = input_text
        .trim()
        .lines()
        .map(|line| line.split(',').collect::<Vec<&str>>()) // line -> commands
...
fn puzzle_b(input: &str) -> String {
            ...
            let points = command
                ...
                .map(|steps| {
                    steps
                        ...
                        .map(|step| {
                            current_point = (
                                current_point.0 + step.0,
                                current_point.1 + step.1,
                                current_point.2 + 1,
                            );
                            current_point
                        })
    ...
    let result = first_wire
        .intersection(second_wire)
        .map(|point| {
            let first = first_wire_repo
                .into_iter()
                .filter(|repo_point| (*point).eq(&(repo_point.0, repo_point.1)))
                .nth(0)
                .unwrap();
            let second = second_wire_repo
                .into_iter()
                .filter(|repo_point| (*point).eq(&(repo_point.0, repo_point.1)))
                .nth(0)
                .unwrap();
